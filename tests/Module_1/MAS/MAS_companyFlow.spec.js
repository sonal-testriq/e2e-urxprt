...
test.describe.serial("MAS Flow", () => {
  ...
  test("TC_003: Verify whether newly created post appears at company's product listing page", async ({
    ...
    await companyMASPage.verifyCompanyListed(MASPostName);
  });
  ...
  test("TC_004: Verify if company can apply on new created post", async ({
    ...
    await companyHomePage.gotoMASViaCard();
    await companyMASPage.openMASCompany(MASPostName);
    await companyMASPage.clickInterestedButton();
    await companyMASPage.acceptMASContract();
    await companyMASPage.verifyInterestSubmitted();
  });

  test("TC_005: Verify if user receives notification when company applies on MAS post", async ({
    userPage,
    userHomePage,
    userMASPage,
  }) => {
  ...
  test("TC_006: Verify if user can see the company's details who applied on MAS post", async ({
    ...
    await userMASPage.verifyInterestedCompany("Shivakumar Padaiyachi");
  });
