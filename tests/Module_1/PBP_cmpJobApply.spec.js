...
test.describe.serial("PBP Flow", () => {
    ...
    test("TC_PBP_006: Verify applied post appears in 'Manage Work' page for 'Company' user", async ({
        ...
        const manageWorkTab = companyPage.getByText('Post & Browse Projects (PBP)', { exact: true })
    ...
    test("TC_PBP_009: Verify whether expert gets notification about offer and expert can accept offer from manage work page", async ({
        ...
        const manageWorkTab = companyPage.getByText('Post & Browse Projects (PBP)', { exact: true })
        await manageWorkTab.click();
        const appliedPost = await companyPage.locator(
          ".post-back",
          {
            hasText: PBPPostName,
          },
        ...
        await expect(appliedPost).toBeVisible();
        await companyPage.locator("//h3[contains(text(),'" + PBPPostName + "')]/parent::div/following-sibling::button").click();
    ...
    test("TC_PBP_010: Verify applied job appears at 'Active job' tabs", async ({
        ...
        const manageWorkTab = companyPage.getByText('Post & Browse Projects (PBP)', { exact: true });
        await manageWorkTab.click();
        const activePost = companyPage.locator("a:has-text('Active projects')");
        await activePost.click();
        await expect(activePost).toBeVisible();
        await companyPage.waitForLoadState("networkidle");
        await companyPage.waitForTimeout(2000);
        const postCard = companyPage.locator(".tab-pane.active .post-back").filter({
          hasText: PBPPostName,
        });
        await expect(postCard).toBeVisible();
      });
    ...
    test("TC_PBP_012: Verify Company is able to submit work for payment", async ({
        ...
        await expect(postCard).toBeVisible();
        await companyPage.locator("//h3[contains(text(),'" + PBPPostName + "')]/parent::div/following-sibling::button").click();
        await companyPage.locator("//div[@class='tab-pane active']//button[contains(text(),'Submit work for payment')]").click();
        await expect(companyPage.locator("//div[@class='modal-content']//p[contains(text(),'Submit work for payment')]")).toBeVisible();
        ...
        const submitWorkButton = companyPage.locator("//button[contains(text(),'Submit Work')]");
        await submitWorkButton.click();
        await companyPage.waitForTimeout(2000);
        const successMessage = await companyPage.locator(".custom-popup.alert.alert-success").textContent();
