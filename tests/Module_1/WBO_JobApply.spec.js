...
test.describe.serial("WBO Flow", () => {
  test("TC_WBO_001: Verify create post from WBO page", async ({
    ...
    await userWBOPage.fillInput(
      "Write a title for your post this ",
      WBOPostName,
    );
    ...
    await nextButton.click();
    await nextButton.click();
    await userWBOPage.expected_deliverables_input.fill(
      "This is test deliverable",
    );
    await userWBOPage.selectDropdown(
    ...
    const postContestButton = userPage.getByRole("button", {
      name: "Post Contest",
    });
    await postContestButton.click();
    await expect(
      userPage.locator("//h6[contains(text(),'Order summary')]"),
    ).toBeVisible();
  ...
  test("TC_WBO_003: Verify newly created post are displayed in My Orders under Posted Posts", async ({
    ...
    await expect(
      userHomePage.post_contest_name_in_all_active_wbo.first(),
    ).toHaveText(WBOPostName);
  });
  ...
  test("TC_WBO_006: Verify submitted entry appears in 'Manage Work' page for 'Expert' user", async ({
    ...
    const manageWorkTab = expertPage.getByText(
      "Win business Opportunities (WBO)",
      { exact: true },
    );
    await manageWorkTab.click();
    const appliedPost = expertPage.locator(".post-back", {
      hasText: WBOPostName,
    });
  ...
  test("TC_WBO_009: Verify submitted entry appears in 'Manage Work' page for 'Company' user", async ({
    ...
    const manageWorkTab = companyPage.getByText(
      "Win business Opportunities (WBO)",
      { exact: true },
    );
    await manageWorkTab.click();
    const appliedPost = companyPage.locator(".post-back", {
      hasText: WBOPostName,
    });
    await expect(appliedPost).toBeVisible();
  });
  ...
  test("TC_WBO_011: Verify user is able to view and add comments on both 'Expert' and 'Company' entries", async ({
    ...
    await expect(
      userPage.locator(".modal-comment .comment-sec", {
  ...
  test("TC_WBO_012: Verify comments appears in 'Notification' for 'Expert' user", async ({
    ...
    const notificationMsg = await expertPage
      .locator(
        "//h6[normalize-space()='New comment on your WBO entry: " +
          WBOPostName +
          "']",
      )
      .first();
  ...
  test("TC_WBO_013: Verify comments appears in 'Notification' for 'Company' user", async ({
    ...
    const notificationMsg = await companyPage
      .locator(
        "//h6[normalize-space()='New comment on your WBO entry: " +
          WBOPostName +
          "']",
