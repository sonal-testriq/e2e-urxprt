...
test.describe.serial("PBP Flow", () => {
    ...
    test("TC_PBP_004: Verify Company User not able to submit proposal", async ({
      companyPage,
      companyHomePage,
      companyPBPPage
    }) => {
      ...
      const newPage = await companyPBPPage.openPost(PBPPostName);
      await companyPBPPage.clickApplyNow(newPage);
      await companyPBPPage.submitProposal(newPage);
      ...
      const actualErrors = await companyPBPPage.getProposalErrors(newPage);
      for (const error of actualErrors) {
        expect(expectedErrors).toContain(error);
      }
    });

    test("TC_PBP_005: Verify Company User is able to submit proposal", async ({
      companyPage,
      companyHomePage,
      companyPBPPage,
    }) => {
    ...
    test("TC_PBP_006: Verify applied post appears in 'Manage Work' page for 'Company' user", async ({
      companyPage,
      companyHomePage,
      companyPBPPage,
    }) => {
      ...
      await companyPBPPage.goToPBPManageWork();
      await companyPBPPage.verifyAppliedPost(PBPPostName);
    });

    test("TC_PBP_007: Verify applied post appears in 'My Orders' page for 'User' after company applies", async ({
      userPage,
      userPBPPage,
    }) => {
      ...
      await userPage.waitForTimeout(1000);
      await userPBPPage.sendOffer_cmp();
    ...
    test("TC_PBP_008: Making payment for sent offer", async ({
      ...
      await userPBPPage.openPostDetails(PBPPostName);
      await userPBPPage.openProposalTab();
      await userPBPPage.clickPaymentButton_cmp();
    ...
    test("TC_PBP_009: Verify whether company gets notification about offer and company can accept offer from manage work page", async ({
      companyPage,
      companyHomePage,
      companyPBPPage,
    }) => {
      ...
      await companyPBPPage.goToPBPManageWork();
      await companyPBPPage.verifyAppliedPost(PBPPostName);
      await companyPBPPage.viewDetailsOfPBPPost(PBPPostName);
      await companyPBPPage.openOffersTab();
      await companyPBPPage.acceptOffer();
      await companyPBPPage.acceptOfferAgreement();
      await companyPage.waitForTimeout(1000);
      await companyPBPPage.verifyOfferAccepted();
    });
    ...
    test("TC_PBP_010: Verify applied job appears at 'Active job' tabs", async ({
      ...
      await expect(companyPage).toHaveURL("https://urxprt.com/en/dashboard");
      await companyPage.waitForLoadState("networkidle");
      await companyPBPPage.goToActivePBPPostInPBPManageWork();
      await companyPage.waitForLoadState("networkidle");
      await companyPage.waitForTimeout(2000);
      await companyPBPPage.verifyPostIsPresentInPBPManageWork(PBPPostName);
    });
    ...
    test("TC_PBP_012: Verify Company is able to submit work for payment", async ({
      companyHomePage,
      companyPage,
      companyPBPPage,
    }) => {
      ...
      await companyPBPPage.verifyPostIsPresentInPBPManageWork(PBPPostName);
      await companyPBPPage.viewDetailsOfPBPPost(PBPPostName);
      await companyPBPPage.clickSubmitWorkForPayment();
      await companyPBPPage.verifySubmitWorkPopup();
      await companyPBPPage.fillInputWithPlaceholder("Enter description here", "This is a test submission for automation");
      await companyPBPPage.uploadWork("testData/sampleImg.jpg");
      await companyPBPPage.submitWork();
      await companyPage.waitForTimeout(1000);
      await companyPBPPage.verifyWorkSubmissionSuccess();
    });

    test("TC_PBP_013: Verify Company is able to view its submitted work", async ({
      companyHomePage,
      companyPage,
      companyPBPPage
    }) => {
      ...
      await companyPBPPage.verifyPostIsPresentInPBPManageWork(PBPPostName);
      await companyPBPPage.viewDetailsOfPBPPost(PBPPostName);
      await companyPBPPage.openSubmissionTab();
      await expect(companyPBPPage.overview_submission_tab).toBeVisible();
      await companyPBPPage.closeSubmissionTab();
      await expect(companyPBPPage.overview_submission_tab).toBeHidden();
    });
