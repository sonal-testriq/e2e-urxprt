...
test.describe.serial("BSM Flow", () => {
  ...
  test("Send request for buying product by expert", async ({
    expertPage,
    expertHomePage,
    expertBSMPage,
  }) => {
    ...
    const BSMpost = await expertPage.locator("div.packaged-img").filter({
      has: expertPage.locator(`h6:text-is("${BSMProductName}")`),
      // has: expertPage.locator(`h6:text-is("Temp BSM post958")`),
    });
  ...
  test("Paying for product by expert(Buyer)  ", async ({
    ...
    await expect(expertPage).toHaveURL(/.*\/myorders\/productforsaledetails\//);
    ...
    await expect(
      expertPage.getByText("BSM Payment Completed").first(),
    ).toBeVisible();
    ...
    const addAddressBtn = expertPage.locator("div.status-right span", {
      hasText: "Add Address",
    });
    ...
    await expertBSMPage.fillInputWithPlaceholder(
      "Enter address",
      "Mumbai, Maharashtra",
    );
    const submitBtn = expertPage.locator("div.modal-content button", {
      hasText: "Submit",
    });
  ...
  test("Marking product 'Deleivered' by expert(Buyer)", async ({
    ...
    const step4 = expertPage.locator("div.status-first").filter({
      has: expertPage.locator("h6", { hasText: "Item Delivered" }),
    });
  ...
  test("Asserting completion of last step of Completing transaction at expert's(buyer) window", async ({
    ...
    const BSMOrders = expertPage.locator("#MyOrderCompleted .order-tabs a", {
      hasText: "Buy & Sell with Market (BSM)",
      exact: false,
    });
