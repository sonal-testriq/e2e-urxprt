...
test.describe.serial("MAS Flow", () => {
  ...
  test("Verify whether newly created post appears at expert's product listing page", async ({
    ...
    await expertPage.goto(pageRoutes.account);
    const masCard = expertPage.locator(".it-dev", {
      has: expertPage.locator("h5", {
        hasText: "Merger and Acquisition (MAS)",
      }),
    });
    ...
    const MASPostCard = expertPage.locator("div.packaged-img h3", {
      hasText: MASPostName,
      exact: false,
    });
    await expect(MASPostCard).toHaveText(MASPostName);
  ...
  test("Verify if expert can apply on new created post", async ({
    ...
    await expertPage
      ...
      .click();
    await expect(expertPage.locator(".modal-overlay-sec.active")).toBeVisible();
    const interestedButton = expertPage.getByRole("button", {
      name: "I'm Interested",
    });
    ...
    expect(expertPage.url()).toContain(
    ...
    const applyButton = expertPage.locator("button", {
      hasText: "Apply",
    });
    await applyButton.click();
    const successToast = expertPage.getByText(
  ...
  test("Verify if user can see the expert's details who applied on MAS post", async ({
    ...
    await expect(expertName).toHaveText("Shivakumar GP");
  });
});
