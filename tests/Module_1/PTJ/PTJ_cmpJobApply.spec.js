...
test.describe.serial("PTJ Flow", () => {
  ...
  test("TC_004: Applying for post by 'company' user", async ({
    ...
    const newPage = await companyPTJPage.openPost(PTJPost_Name);
    await companyPTJPage.applyForJob(newPage);
    await companyPTJPage.submitProposal(newPage);
    await companyPTJPage.verifyProposalSubmitted(newPage);
  });
  ...
  test("TC_005: Verify applied post appears in 'Manage Work' page for 'Company' user", async ({
    ...
    const manageWorkTab = companyPage.locator("a", { hasText: "Part time Job (PTJ)" });
    await manageWorkTab.click();
    const appliedPost = companyPage.locator(".post-back.recent-back.recent-first", { hasText: PTJPost_Name });
  ...
  test("TC_006: Verify applied post appears in 'My Orders' page for 'User' after company applies", async ({
    ...
    await userPTJPage.sendOfferToCompany("Shivakumar Padaiyachi");
  ...
  test("TC_007: Making payment for sent offer", async ({ 
    ...
    await userPTJPage.openProposalForPayment(PTJPost_Name, "Shivakumar Padaiyachi");
  ...
  test("TC_008: Verify whether company gets notification about offer and company can accept offer from manage work page", async ({
    ...
    await companyHomePage.gotoDashboardPage();
    await expect(companyPage).toHaveURL("https://urxprt.com/en/dashboard");
    await companyPage.locator("a", { hasText: "Part time Job (PTJ)" }).click();
    await companyPTJPage.openManageWorkPost(PTJPost_Name);
    await companyPTJPage.openOffersTab();
    await companyPTJPage.clickAcceptOffer();
    await companyPTJPage.acceptStartJobContract()
  ...
  test("TC_009: Verify applied job appears at 'Active job' tabs", async ({
    ...
    await companyHomePage.openPTJManageWork();
    await expect(companyPage).toHaveURL("https://urxprt.com/en/dashboard/managework/jobs");
    await companyPTJPage.openActiveJobsTab();
    await companyPTJPage.verifyActiveJob(PTJPost_Name);
    await companyPTJPage.openActiveJob(PTJPost_Name);
  ...
  test("TC_010: Verify 'Log time' button navigates to timesheet", async ({
    ...
    await companyPTJPage.openActiveJobsTab();
    await companyPTJPage.openActiveJob(PTJPost_Name);
    await companyPTJPage.clickLogTime();
    await companyPage.waitForTimeout(2000);
  ...
  test("TC_011: Logging time for job", async ({ 
    ...
    await companyPTJPage.openActiveJob(PTJPost_Name);
    await companyPTJPage.clickLogTime();
    await companyPTJPage.enterWorkLogDescription(
      "This is my first work logging"
    );
    await companyPTJPage.selectStartTime("10", "30", "AM");
    await companyPTJPage.selectEndTime("4", "30", "PM");
    const current_date = await companyHomePage.getCurrentDate();
    await companyPTJPage.selectWorkDate(current_date);
    await companyPTJPage.verifyTotalHours("6:00");
    await companyPTJPage.clickLogTimeButton();
    await companyPage.waitForTimeout(2000);
  });
  ...
  test("TC_012: to verify whether pending sheet has logged time in sheet ", async ({
    ...
    await companyPTJPage.openActiveJob(PTJPost_Name);
    await companyPTJPage.clickLogTime();
    await companyPTJPage.verifyPendingSheetsTab();
    const currentDate = await companyHomePage.getCurrentDate_DMY();
    const dateLocator =
      await companyPTJPage.verifyLoggedDate(
        currentDate
      );
    await companyPTJPage.verifyLoggedHours(
      dateLocator,
      "Total : 06:00:00"
    );
    await companyPTJPage.verifyLogEntry(
      "This is my first work logging",
      "10:30 AM",
      "4:30 PM",
      "06:00:00",
